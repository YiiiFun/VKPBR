...
bool Renderer::Initialize(const std::string& appName, bool enableValidationLayers) {
  ...
  if (!createComputeResources()) {
    LOGE("Failed to create compute resources");
    return false;
  }
  LOGI("Compute resources created successfully");
...
bool Renderer::createLogicalDevice(bool enableValidationLayers) {
  ...
  try {
    ...
    features2.features.shaderInt64 = coreSupported.shaderInt64 ? vk::True : vk::False;
    ...
    if (hasRobust2) {
      if (robust2Supported.robustBufferAccess2 && coreSupported.robustBufferAccess) {
        robust2Enable.robustBufferAccess2 = vk::True;
        features2.features.robustBufferAccess = vk::True;
      }
    ...
    if (hasRQ && rayQuerySupported.rayQuery && coreSupported.shaderInt64) {
