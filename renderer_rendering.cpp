...
bool Renderer::createSwapChain() {
  try {
    ...
    // Use the surface's current transform when required by the window system.
...
void Renderer::prepareFrameUboTemplate(CameraComponent* camera, ImGuiSystem* imguiSystem) {
  ...
  // The basic/legacy shader reuses it as the Blinn-Phong toggle only when PBR is disabled.
  const bool useBasicLighting = imguiSystem && !imguiSystem->IsPBREnabled();
  const bool useBlinnPhong = useBasicLighting && imguiSystem->IsBlinnPhongEnabled();
  frameUboTemplate.padding1 = useBlinnPhong ? 2.0f : (useForwardPlus ? 1.0f : 0.0f);
...
void Renderer::Render(const std::vector<Entity *>& entities, CameraComponent* camera, ImGuiSystem* imguiSystem) {
  ...
  prepareFrameUboTemplate(camera, imguiSystem);
  ...
  const bool needsAccelerationStructure = NeedsAccelerationStructure();
  if (needsAccelerationStructure && rayQueryEnabled && accelerationStructureEnabled &&
      (asOpportunisticRebuildEnabled || IsLoading())) {
  ...
  if (imguiSystem && imguiSystem->IsVisible() && !imguiSystem->IsFrameRendered() && !IsLoading()) {
    if (ImGui::Begin("Renderer")) {
      ...
      if (currentRenderMode == RenderMode::Rasterization) {
        ...
        // Lighting Controls - Phong is the default legacy/basic model.
        bool useBasicLighting = imguiSystem && !imguiSystem->IsPBREnabled();
        bool useBlinnPhong = useBasicLighting && imguiSystem && imguiSystem->IsBlinnPhongEnabled();
        bool usePhong = useBasicLighting && !useBlinnPhong;

        if (ImGui::Checkbox("Use Basic Lighting (Phong)", &usePhong)) {
          if (usePhong) {
            // Mutually exclusive with Blinn-Phong.
            imguiSystem->SetPBREnabled(false);
            imguiSystem->SetBlinnPhongEnabled(false);
          } else {
            imguiSystem->SetPBREnabled(true);
            imguiSystem->SetBlinnPhongEnabled(false);
          }
          std::cout << "Lighting mode: " << (usePhong ? "Basic Phong" : "BRDF/PBR") << std::endl;
        }

        if (ImGui::Checkbox("Use Basic Lighting (Blinn-Phong)", &useBlinnPhong)) {
          if (useBlinnPhong) {
            // Mutually exclusive with Phong.
            imguiSystem->SetPBREnabled(false);
            imguiSystem->SetBlinnPhongEnabled(true);
            std::cout << "Lighting mode: Basic Blinn-Phong" << std::endl;
          } else {
            imguiSystem->SetPBREnabled(true);
            imguiSystem->SetBlinnPhongEnabled(false);
            std::cout << "Lighting mode: BRDF/PBR" << std::endl;
          }
        }

        useBasicLighting = imguiSystem && !imguiSystem->IsPBREnabled();
        useBlinnPhong = useBasicLighting && imguiSystem->IsBlinnPhongEnabled();
        if (!useBasicLighting) {
          ImGui::Text("Status: BRDF/PBR pipeline active");
          ImGui::Text("All models rendered with physically-based lighting");
        } else if (useBlinnPhong) {
          ImGui::Text("Status: Basic Blinn-Phong pipeline active");
          ImGui::Text("Phong diffuse/ambient with half-vector specular");
        } else {
          ImGui::Text("Status: Basic Phong pipeline active");
          ImGui::Text("Phong diffuse/ambient with reflection-vector specular");
        }
        ...
        if (rayQueryEnabled && accelerationStructureEnabled) {
          if (ImGui::Checkbox("RayQuery shadows (raster)", &enableRasterRayQueryShadows) &&
              enableRasterRayQueryShadows && !*tlasStructure.handle) {
            RequestAccelerationStructureBuild("raster ray-query shadows enabled");
          }
        } else {
