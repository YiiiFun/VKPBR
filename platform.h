...
class Platform {
  public:
    ...
    /**
     * @brief Set a callback for mouse wheel input.
     * @param callback Receives the vertical wheel offset.
     */
    virtual void SetScrollCallback(std::function<void(float)> callback) = 0;
...
class DesktopPlatform final : public Platform {
  ...
  public:
    ...
    void SetScrollCallback(std::function<void(float)> callback) override;
...
std::unique_ptr<Platform> CreatePlatform(Args&&...) {
