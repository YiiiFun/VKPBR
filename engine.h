...
class Engine
{
  public:
	...
	/**
	 * @brief Handles vertical mouse wheel input for camera dolly movement.
	 */
  ...
  private:
	...
	struct CameraControlState
	{
		...
		float     lastRightMouseX         = 0.0f;
		float     lastRightMouseY         = 0.0f;
		float     yaw                     = 0.0f;
		float     pitch                   = 0.0f;
		...
		bool      rightStartedOnImGui     = false;
		bool      isFirstFrameOfRightInteraction = true;
	...
	/**
	 * @brief Draw the camera control/debug panel.
	 */
	void DrawCameraControlPanel();

	/**
	 * @brief Resolve a desired camera movement against scene mesh AABBs.
	 */
	glm::vec3 ResolveCameraCollision(const glm::vec3 &currentPosition, const glm::vec3 &desiredPosition);

	/**
	 * @brief Check whether a camera sphere at the given position intersects scene mesh AABBs.
	 */
	bool IsCameraPositionBlocked(const glm::vec3 &position) const;
