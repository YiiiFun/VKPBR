...
bool Renderer::createTextureImage(const std::string& texturePath_, TextureResources& resources, bool cachePixels) {
  try {
    ...
    std::unique_ptr<stbi_uc, decltype(&stbi_image_free)> decodedPixels(nullptr, stbi_image_free);
    ...
    if (isKtx2) {
      ...
      decodedPixels.reset(stbi_load(resolvedPath.c_str(), &texWidth, &texHeight, &texChannels, STBI_rgb_alpha));
      if (!decodedPixels) {
        std::cerr << "Failed to decode texture '" << resolvedPath << "': "
                  << stbi_failure_reason() << std::endl;
        return false;
      }

      pixels = decodedPixels.get();
...
bool Renderer::LoadTexture(const std::string& texturePath, bool cachePixels) {
  ...
  bool success = createTextureImage(resolvedId, tempResources, cachePixels);
  if (success)
    return true;
...
void Renderer::ProcessPendingMeshUploads() {
  ...
  if (!!*uploadsTimeline && *uploadsTimeline != vk::Semaphore{}) {
    ...
    if (anyCompleted && NeedsAccelerationStructure()) {
        ...
        if (!memJobs.empty()) {
          try {
            ...
            if (!items.empty()) {
              ...
              vk::raii::Fence fence(device, vk::FenceCreateInfo{});
              uint64_t signalValue = 0;
              bool canSignal = !!*uploadsTimeline; {
                std::lock_guard<std::mutex> lock(queueMutex);
...
void Renderer::generateMipmaps(vk::Image image,
  ...
  for (uint32_t i = 1; i < mipLevels; ++i) {
    ...
    vk::ImageMemoryBarrier2 prevToRead2{
