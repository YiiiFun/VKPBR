...
struct SSAOUniformBufferObject {
  ...
  alignas(16) glm::vec4 screenRadiusBias;
  alignas(16) glm::vec4 intensitySamples;
...
class Renderer {
  public:
    ...
    bool NeedsAccelerationStructure() const {
      return currentRenderMode == RenderMode::RayQuery || enableRasterRayQueryShadows;
    }
    ...
    // Ray Query static-only mode (disable animation updates and TLAS refits to render a static opaque scene)
    ...
    void RequestAccelerationStructureBuild(const char* reason) {
      ...
      // Explicit requests bypass the freeze to ensure dynamic objects are added.
  ...
  private:
    ...
    vk::raii::DescriptorSetLayout compositeDescriptorSetLayout = nullptr;
    std::vector<vk::raii::DescriptorSet> compositeDescriptorSets;

    // Screen-space ambient occlusion compute passes.
    ...
    std::vector<vk::raii::DescriptorSet> ssaoDescriptorSets;
    std::vector<vk::raii::DescriptorSet> ssaoBlurDescriptorSets;

    // GTAO pipeline (shares SSAO descriptor set layout and resources).
    ...
    // SSAO render targets, one pair per frame-in-flight.
    vk::Format ssaoFormat = vk::Format::eR16Sfloat;
    std::vector<vk::raii::Image> ssaoRawImages;
    std::vector<std::unique_ptr<MemoryPool::Allocation>> ssaoRawImageAllocations;
    std::vector<vk::raii::ImageView> ssaoRawImageViews;
    std::vector<vk::ImageLayout> ssaoRawImageLayouts;
    std::vector<vk::raii::Image> ssaoBlurImages;
    std::vector<std::unique_ptr<MemoryPool::Allocation>> ssaoBlurImageAllocations;
    std::vector<vk::raii::ImageView> ssaoBlurImageViews;
    std::vector<vk::ImageLayout> ssaoBlurImageLayouts;
    vk::raii::Sampler ssaoSampler{nullptr};
    ...
    // - Animation updates are suppressed by the Engine (input/Update hook)
    ...
    bool textureMipmapsEnabled = true;
    float textureMipLodBias = 0.0f;
    bool textureSamplersDirty = false;
    ...
    bool ssaoEnabled = false;        // derived: true when aoMode != Off
    ...
    void refreshTextureSamplers(const std::vector<Entity *>& entities);
	...
	void prepareFrameUboTemplate(CameraComponent *camera, ImGuiSystem *imguiSystem = nullptr);
